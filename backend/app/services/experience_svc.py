...
# 营销活动方案/复盘/报表文件。注意：上传的多是【未执行的方案】——有目标/打法/预算
# 但没有实际效果数据。强制要求 result_metrics 会把方案类文档全部拒掉（真实事故：
# 「理县-都江堰营销活动方案」「便民快巴宣传方案」均因此判无价值）。方案本身的打法
# 就是可复用经验，result_metrics 只对含实际效果数据的文档强制。
CAMPAIGN_DISTILL_PROMPT = (
    "你是营销活动经验提炼器。从营销活动文件中提炼【可复用的营销经验】："
    "营销活动中形成的、未来同类活动可复用的策略、打法或教训。"
    "营销活动方案（未执行、含目标与打法）和复盘报告（已执行、含效果数据）都可提炼。\n"
    "\n"
    "## 从文件中提取：\n"
    "1. 活动名称、活动时间/周期、渠道、预算、目标人群；\n"
    "2. 核心打法：渠道组合、预算分配、玩法（优惠券/满减/直播/地推/社群裂变等）、宣传动作；\n"
    "3. 预期目标或实际效果：GMV、ROI、转化率、订单量、注册数、知晓率等；\n"
    "4. 复盘结论（若为复盘报告）：成功原因、失败教训。\n"
    "\n"
    "## 无价值场景（title 必须为 null）：\n"
    "1. 文件不含实际营销活动信息（空内容、无法识别活动）；\n"
    "2. 只有活动名称/日期，没有任何打法、策略或效果信息，无法形成可复用经验。\n"
    "\n"
    "## 输出要求：\n"
    "- 有价值 → title 简洁概括（≤20 字），summary 提炼可复用要点，content 记录活动全貌（背景/目标/打法/预算/效果）；\n"
    "- event_time：活动开始日期（无明确日期时置 null）；\n"
    "- result_metrics：文档含实际效果数据（GMV/ROI/转化率等）时必填；纯方案（未执行、无效果数据）可省略，预期目标写入 content；\n"
    "- 若文档既有完整打法又有实际效果数据，则 result_metrics 必须填写。\n"
    "\n"
    "文件内容：\n{text}"
)
